# Crie um programa que seja uma API de um contador de números. Esta API irá manter o número em sua memória, e esta irá iniciar com o valor 0 (zero).
...
memoria = {"valor_numero": 0}

@app.post("/contador")
def novo_numero():
    valor = request.get_json("numero")
    memoria["valor_numero"] = valor["numero"]
    return "", 201

@app.get("/contador")
def retorna_valor():
    return {"numero": memoria["valor_numero"]}, 200

@app.put("contador/incrementa")
def incrementa():
    memoria["valor_numero"] += 1
    return " ", 202
